...
resource "azurerm_key_vault_access_policy" "tde_policy" {
  ...
  tenant_id       = azurerm_mssql_server.mysql_server.identity[0].tenant_id
  object_id       = azurerm_mssql_server.mysql_server.identity[0].principal_id
  key_permissions = var.tde_key_permissions
}

resource "azurerm_mssql_server_transparent_data_encryption" "mysql_server" {
  count = var.tde_encryption_enabled ? 1 : 0
...
resource "azurerm_mssql_server_security_alert_policy" "mysql_server" {
  count = var.mssql_defender_state == null ? 0 : 1

  resource_group_name = azurerm_mssql_server.mysql_server.resource_group_name
  server_name         = azurerm_mssql_server.mysql_server.name
